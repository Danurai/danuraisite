...
function swatch (c) {
  return '<div class="col-sm-2 px-0">'
    ...
    + '<div>' + (typeof c.oldgw == 'undefined' ? '(#NA)' : c.oldgw) + '</div>'
    + '</div>' 
    + '</div>';
}
...
$('#colours').on('click','.sample', function () {
  compare.push({name: $(this).data('name'), range: $(this).data('range')});
  write_comparison();
});
$('#comparison').on('click','.sample', function () {
  compare.splice($.inArray({name: $(this).data('name'), range: $(this).data('range')}, compare),1);
  write_comparison();
});
...
$('.sortable').on('click', function () {
  var fld = ["name","range","hex","h","s","l"][this.cellIndex];
  var dir = "asec";
