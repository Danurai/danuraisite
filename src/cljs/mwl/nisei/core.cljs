...
(go
  (reset! cycles   (-> (<! (http/get "/api/cycles"))   :body :data))
  (reset! packs    (-> (<! (http/get "/api/packs"))    :body :data))
  (reset! mwls     (-> (<! (http/get "/api/mwl"))      :body :data))
  (reset! types    (-> (<! (http/get "/api/types"))    :body :data))
  (let [cardsapi (<! (http/get "/api/cards"))]
  ...
  (reset! colours 
    (apply merge 
      (map 
        ...
        (-> (<! (http/get "/api/factions")) :body :data)))))
